    ...
    <form id="salesInfoForm">
        <div id="formwrap" class="bts" v-if="action">
            <div id="salesInfoArea">
                    <div class="mt20" id="area-left">
                        ...
                        <div id="shortImages" @mouseleave="initMainPhoto">
                            <div v-for="item in photos" :key="item.file_server_path">
                                <img :src="item.file_server_path" @mouseover='swapMainPhoto(item.file_server_path)' />
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="area-right" class="bts">
                        <table>
                            <tr>
                                <td>모델명</td>
                                <td><input type="text" class="form-control" name="model_nm" v-model="info.model_nm"/></td>
                                <td colspan="2"></td>
                            </tr>
                            <tr>
                                <td>모델번호</td>
                                <td><input type="text" class="form-control" name="model_code" v-model="info.model_code"/></td>
                                <td>제조사</td>
                                <td><input type="text" class="form-control" name="mfcomp" v-model="info.mfcomp"/></td>
                            </tr>
                            <tr>
                                <td>제품명</td>
                                <td><input type="text" class="form-control" name="sales_nm" v-model="info.sales_nm"/></td>
                                <td>제품가격</td>
                                <td><input type="text" class="form-control" name="price" v-model="info.price" @keydown="fFilterNumber"/>
                                </td>
                            </tr>
                            <tr>
                                <td colspan="4">
            ...
            <div class="salesMngBtnArea text-right" style="margin-right: 15px; margin-bottom: 30px">
                <button type="button" class="btn btn-primary mr-3" v-if="action == 'NEW'" @click="manage('POST')">등록</button>
                <button type="button" class="btn btn-primary mr-3" v-if="action == 'INFO'" @click="manage('POST', info.sales_id)">저장</button>
...
export default {
    ...
    methods: {
        ...
        this.axios
            ...
            .then((resp) => {
                ...
                if(action=='NEW'){
                    ...
                    this.axios
                        .post("/commonproc/comcombo.do",new URLSearchParams({group_code:"proCD"}))
                        .then((resp)=>{
                            let data=resp.data;
        ...
        manage:function(action,salesID){
            ...
            let confirmMsg=(action=='POST'&&salesID==0)?'등록하시겠습니까?':action=='POST'?'정말로 수정하시겠습니까?':'정말로 삭제하시겠습니까? 삭제한 정보는 되돌릴 수 없습니다';
            if(confirm(confirmMsg)){
                ...
                this.axios(
                    ...
                    if(result==1)
                        msg=(action=='POST'&&salesID==0)?'정상적으로 등록되었습니다':action=='POST'?'정상적으로 수정되었습니다':'정상적으로 삭제되었습니다';
                    alert(msg);
                    vue.close();
