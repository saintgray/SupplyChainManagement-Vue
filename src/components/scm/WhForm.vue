	...
	<div id="formwrap" v-if='action'>
		...
		<table id="formtable">
			<tr>	
				<td v-if='action=="INFO"'>
					<div class="input-group">
						...
						<input type="text" v-model='info.wh_id' class="form-control" readonly>
					</div>
				</td>					
				<td>
					<div class="input-group">
				...
				<td v-if='action=="NEW"'>
					...
					<select id="location" v-model='info.wh_loc' class="form-control">
						<option v-for='items in locationList' :value="items.dtl_cod" :key="items.dtl_cod">{{items.dtl_cod_nm}}</option>
					</select>
				</td>
			</tr>	
			<tr>
				<td>
					<div class="input-group">
						...
						<select id="advisor" class="h-auto" v-model="info.loginID" @change='advisorChoice($event)'>
							<option v-for='item in advisorList' :value="item.dtl_cod" :key="item.dtl_cod">{{item.dtl_cod_nm}}</option>
						</select>
					</div>
				</td>	
				<td>
					<div class="input-group" id='phoneArea'>
						...
						<input type="text" class="form-control text-center" ref="head" readonly>
						<span class="input-group-text bg-white border-white">-</span>
						<input type="text" class="form-control text-center" ref="mid" readonly>
						<span class="input-group-text bg-white border-white">-</span>
						<input type="text" class="form-control text-center" ref="end" readonly>
					</div>
				</td>
			</tr>
			<tr>
				<td colspan="2">
					<div class="input-group">
						...
						<input type="text" class="form-control text-center" v-model='info.email' readonly>
					</div>
				</td>
			</tr>	
			<tr>
				<td colspan="2">
					<div class="input-group">
						...
						<div class="form-inline">
							...
							<input type="button" id="findAddress" class="btn mx-2" value='주소찾기' @click='findZip'>
						</div>
					</div>
				</td>
			</tr>
			...
			<tr>
				<td colspan="2">
					<span>상세주소</span>
					<input id="dtAddress" type="text" class="form-control"  v-model='info.dtAddress'>
				</td>
			</tr>
		</table>
		...
		<div class="my30 d-flex justify-content-end" style="margin-bottom: 50px;">
			...
			<button type="button" class="btn btn-primary mx-2" id="btnRegister" v-if='action=="NEW"' @click='manage("POST")'>등록</button>
		
			<template v-if='action=="INFO"'>
				<button type="button" class="btn btn-primary mx-2" @click='manage("POST",info.wh_id)'>저장</button>
				<button type="button" class="btn btn-danger mx-2" @click='manage("DELETE",info_wh_id)'>삭제</button>
			</template>
