<template>
    ...
    <div id="lnb_area">
        ...
        <ul class="lnbMenu">
            <template
                v-for="(list, index) in loginInfo.menulist"
                :key="list.mnu_id"
            >
                <li>
                    <dl>
                        <!-- 대분류 메뉴 -->
                        <dt>
                            <a class="lnbBtn menu005" :class="list.isShow == true ? 'on' : ''" href="#" @click="slideDown(index, $event)" >{{ list.mnu_nm }}
                            </a>
                        </dt>
                        ...
                        <slide v-model="list.isShow" :duration="400">    
                            <dd>
                                <template v-for="i in list.nodeList" :key="i.mnu_id">
                                    ...
                                    <router-link :to="'/dashboard'+i.mnu_url.replace('.do','')">- {{i.mnu_nm}}</router-link>
                                </template>
                            </dd>
                        </slide>
